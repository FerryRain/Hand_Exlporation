...
def move_to_new(robot, controller, quat, gpis, hand_pos_now, entities, contact_force, alpha_base=0.025, beta_base=0.06):
    ...
    d_Normal_tensor = torch.tensor(d_Normal, device="cuda:0").reshape(-1,3)
    q_target = quat_align_z_to_vector(d_Normal_tensor).reshape(-1, 4)
...
def run_simulator(sim: sim_utils.SimulationContext, entities):
    ...
    frame_marker_cfg.markers["frame"].scale = (.03, .03, .03)
    ...
    gpis = init_normal_HE_GPIS_model_2(temp_min, temp_max, temp_bb_min, temp_bb_max, res=100, grid_count=6, store_path="../Results/Exploration_env_stage7_")
    ...
    while simulation_app.is_running():
        ...
        if len(estimated_surface) > 0 and count % 50 == 0:
            target_pos, target_quat = move_to_new(robot, controller, target_quat, gpis,
