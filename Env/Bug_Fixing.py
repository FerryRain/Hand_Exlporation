...
def run_simulator(sim: sim_utils.SimulationContext, entities):
    ...
    joint_vel_limits = torch.full((1, len(robot.data.joint_names)), .001)
    joint_effort_limits = torch.full((1, len(robot.data.joint_names)), 0.01)
    # robot.data.joint_vel_limits = joint_vel_limits
    # robot.data.joint_effort_limits = joint_effort_limits
    # robot.data.soft_joint_vel_limits = joint_vel_limits
    ...
    while simulation_app.is_running():
        ...
        if get_exploration_status(count):  # time to next exploration
            ...
            def random_unit_quaternion(device='cuda'):
                import math
                ...
                q[0] = torch.sqrt(1 - u1) * torch.sin(2 * math.pi * u2)  # x
                q[1] = torch.sqrt(1 - u1) * torch.cos(2 * math.pi * u2)  # y
                q[2] = torch.sqrt(u1) * torch.sin(2 * math.pi * u3)  # z
                q[3] = torch.sqrt(u1) * torch.cos(2 * math.pi * u3)  # w

                # optional: normalize again for safety
                # q = q / torch.linalg.norm(q)

                return q
            target_quat = random_unit_quaternion()
            target_quat_check = check_quat_validity(target_quat)
            if target_quat_check is not None:
                target_quat = target_quat_check

            grasp_joint_pos = robot.data.default_joint_pos.clone()
            # robot.set_joint_position_target(grasp_joint_pos)
